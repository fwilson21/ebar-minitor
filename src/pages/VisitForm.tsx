...
// Radio (en metros, desde las coordenadas registradas de la estación) dentro del cual se acepta
// que el operador está en el sitio. Amplio a propósito: el GPS del celular pierde mucha precisión
// dentro/cerca de las cámaras de bombeo (concreto y metal) y sin señal de datos no tiene A-GPS,
// así que un radio chico bloqueaba a operadores que sí estaban en la estación.
const DISTANCIA_MAXIMA_METROS = 1000;
...
export function VisitForm() {
  ...
  // movibles como Inicio/Estaciones. Es exclusiva del administrador real (ver migración 0053),
  ...
  function descartarYSalir() {
    ...
    if (confirmar) descartarBorrador(() => navigate(-1));
  }
  ...
  async function manejarGuardar() {
    ...
    const payload: VisitaInput = {
      ...
      // "Fuera de servicio" ya no oculta el resto de la hoja de visita, así que estos campos se
      // guardan igual que en cualquier otra visita si el operador los llenó — solo nivel_tanque
      // necesita un valor de respaldo ('medio') porque la columna no admite vacío y, a diferencia
  ...
  // esto un operador parado justo en el borde del radio podía quedar bloqueado por error.
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="text-right flex-shrink-0">
          ...
          <div className="flex flex-col items-end gap-1.5 mt-2">
            <button
              ...
              onClick={() => pausarYSalir(() => navigate(-1))}
            >
              ...
              <div className="tarjeta p-4 space-y-3">
                ...
                {esFueraDeServicio && (
                  <p className="text-xs text-slate-600 bg-panel-900 border border-panel-600 rounded-lg px-3 py-2">
                    La estación quedó marcada como <strong>Fuera de servicio</strong>: el resto de
                    la hoja de visita (nivel de tanque, bombas, equipos) queda habilitado pero es
                    opcional — repórtalo si aplica.
                  </p>
                )}
      ...
      {pasoConfirmacion === 0 && modoEdicion && !hayCambios && (
        <button type="button" onClick={() => navigate(-1)} className="boton-secundario w-full">
          Salir
        </button>
      )}
